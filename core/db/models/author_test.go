...
func testAuthorsExists(t *testing.T) {
	...
	e, err := AuthorExists(ctx, tx, o.AuthorID)
	if err != nil {
...
func testAuthorsFind(t *testing.T) {
	...
	authorFound, err := FindAuthor(ctx, tx, o.AuthorID)
	if err != nil {
		t.Error(err)
	}
...
func testAuthorToManyAuthorpageratings(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, authorpageratingDBTypes, false, authorpageratingColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, authorpageratingDBTypes, false, authorpageratingColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}

	queries.Assign(&b.AuthorID, a.AuthorID)
	queries.Assign(&c.AuthorID, a.AuthorID)
	...
	check, err := a.Authorpageratings().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := AuthorSlice{&a}
	if err = a.L.LoadAuthorpageratings(ctx, tx, false, (*[]*Author)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Authorpageratings); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Authorpageratings = nil
	if err = a.L.LoadAuthorpageratings(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Authorpageratings); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testAuthorToManyAuthorreviews(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.Authorreviews().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := AuthorSlice{&a}
	if err = a.L.LoadAuthorreviews(ctx, tx, false, (*[]*Author)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Authorreviews); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Authorreviews = nil
	if err = a.L.LoadAuthorreviews(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Authorreviews); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testAuthorToManyBooks(t *testing.T) {
	...
	_, err = tx.Exec("insert into \"authorsbooks\" (\"author_id\", \"book_id\") values ($1, $2)", a.AuthorID, b.BookID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"authorsbooks\" (\"author_id\", \"book_id\") values ($1, $2)", a.AuthorID, c.BookID)
	if err != nil {
		t.Fatal(err)
	}
...
func testAuthorToManyAddOpAuthorpageratings(t *testing.T) {
	var err error
	...
	foreigners := []*Authorpagerating{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, authorpageratingDBTypes, false, strmangle.SetComplement(authorpageratingPrimaryKeyColumns, authorpageratingColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Authorpagerating{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddAuthorpageratings(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if !queries.Equal(a.AuthorID, first.AuthorID) {
			t.Error("foreign key was wrong value", a.AuthorID, first.AuthorID)
		}
		if !queries.Equal(a.AuthorID, second.AuthorID) {
			t.Error("foreign key was wrong value", a.AuthorID, second.AuthorID)
		}
		...
		if a.R.Authorpageratings[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Authorpageratings[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Authorpageratings().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testAuthorToManySetOpAuthorpageratings(t *testing.T) {
	var err error
	...
	err = a.SetAuthorpageratings(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetAuthorpageratings(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !queries.Equal(a.AuthorID, d.AuthorID) {
		t.Error("foreign key was wrong value", a.AuthorID, d.AuthorID)
	}
	if !queries.Equal(a.AuthorID, e.AuthorID) {
		t.Error("foreign key was wrong value", a.AuthorID, e.AuthorID)
	}
	...
	if a.R.Authorpageratings[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.Authorpageratings[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testAuthorToManyRemoveOpAuthorpageratings(t *testing.T) {
	var err error
	...
	err = a.AddAuthorpageratings(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveAuthorpageratings(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.Authorpageratings) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.Authorpageratings[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.Authorpageratings[0] != &e {
		t.Error("relationship to e should have been preserved")
	}
}

func testAuthorToManyAddOpAuthorreviews(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddAuthorreviews(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.Authorreviews[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Authorreviews[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Authorreviews().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testAuthorToManySetOpAuthorreviews(t *testing.T) {
	var err error
	...
	err = a.SetAuthorreviews(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetAuthorreviews(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.Authorreviews[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.Authorreviews[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testAuthorToManyRemoveOpAuthorreviews(t *testing.T) {
	var err error
	...
	err = a.AddAuthorreviews(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveAuthorreviews(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.Authorreviews) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.Authorreviews[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.Authorreviews[0] != &e {
		t.Error("relationship to e should have been preserved")
	}
}
...
var (
	authorDBTypes = map[string]string{`AuthorID`: `uuid`, `Fullname`: `character varying`, `Biography`: `character varying`, `DateOfBirth`: `date`, `PlaceOfBirth`: `character varying`, `DateOfDeath`: `date`, `PlaceOfDeath`: `character varying`}
	_             = bytes.MinRead
)
